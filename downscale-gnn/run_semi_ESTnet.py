import numpy as np
import torch, pickle, data_semi, utils
import network_semi_ESTnet as network_semi
import os
import wandb
...
if job_id:
    output_dir = f'./semi_supervised_ESTnet_{timestamp}_job{job_id}'
else:
    output_dir = f'./semi_supervised_ESTnet_{timestamp}'
...
def main():
    ...
    wandb.init(
        ...
        name=f'semi_supervised_ESTnet_{modelName}',
        config={
            **dataParam,
            **modelParam,
            ...
            'model_type': 'semi_supervised_ESTnet',
        }
    )
    
    model = (network_semi.GNN_ESTNet(modelParam)).to(device)
    ...
    for epoch in range(EPOCH,nEpoch):
        trainLoss,trainRMSE,_,_ = network_semi.train(
            ...
            metadata['nNodes'], metadata['nNodes_labeled'], dynamic_dim, static_dim
        )
