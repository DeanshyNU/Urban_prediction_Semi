...
def main():
    ...
    with open(f'./{model_path}_log', 'a') as f:
        ...
        print("  UQ参数: n_augments=5, epsilon=1e-2, weight_clamp=10.0", file=f)
        ...
        print("  无标签损失权重: lambda_U=3（带ramp-up, ramp_ep=100）", file=f)
    ...
    for epoch in range(EPOCH, nEpoch):
        ...
        trainLoss, trainRMSE, _, _, epoch_debug = train_fixmatch_multiple_weak(
            trainLoader, weak_loaders, strong_loader, model, lossFn, loss_unlabel_fn, opt, scheduler, device, metadata['nNodes'], lambda_U=3.0 * ramp
        )
        ...
        with open(f'./{model_path}_log', 'a') as f:
            ...
            # 调试信息
            print(f"  [DEBUG] labeled_loss={epoch_debug.get('debug/labeled_loss_mean',0):1.4e} | unlabeled_loss={epoch_debug.get('debug/unlabeled_loss_mean',0):1.4e}", file=f)
            print(f"  [DEBUG] pseudo_label: mean={epoch_debug.get('debug/pseudo_label_mean_mean',0):1.4f}, std={epoch_debug.get('debug/pseudo_label_std_mean',0):1.4f}", file=f)
            print(f"  [DEBUG] pred_std: min={epoch_debug.get('debug/pred_std_min_mean',0):1.6f}, max={epoch_debug.get('debug/pred_std_max_max',0):1.6f}, mean={epoch_debug.get('debug/pred_std_mean_mean',0):1.6f}", file=f)
            print(f"  [DEBUG] weights: min={epoch_debug.get('debug/weight_min_mean',0):1.4f}, max={epoch_debug.get('debug/weight_max_max',0):1.4f}", file=f)
            print(f"  [DEBUG] logits_strong: mean={epoch_debug.get('debug/logits_strong_mean_mean',0):1.4f}, std={epoch_debug.get('debug/logits_strong_std_mean',0):1.4f}", file=f)
            print(f"  [DEBUG] grad_norm: before_clip_max={epoch_debug.get('debug/grad_norm_before_clip_max',0):1.4f}, after_clip_max={epoch_debug.get('debug/grad_norm_after_clip_max',0):1.4f}", file=f)
            print(f"  [DEBUG] point_wise_loss_max={epoch_debug.get('debug/point_wise_loss_max_max',0):1.4e}", file=f)
