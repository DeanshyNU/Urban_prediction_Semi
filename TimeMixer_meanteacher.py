"""
TimeMixer + Mean Teacher 主程序
使用 TimeMixer 替换 GNN 作为 backbone，保持 Mean Teacher 半监督学习框架
"""
import os
import torch
import pickle
...
from backbones import TimeMixerModel  # 使用 TimeMixer 替换 GNN
...
def main():
    ...
    modelParam = {
        'HLD': 128,           # d_model
        'nMLP': 2,            # 保留（TimeMixer不使用，但保持兼容）
        'nGNN': 2,            # PDM block数量 (e_layers)
        'iDim': metadata['iDim'],
        'oDim': metadata['oDim'],
        # TimeMixer 特有参数
        'nNodes': metadata['nNodes'],  # 站点数 = token数
        'nhead': 8,           # 保留（TimeMixer不使用注意力，但保持兼容）
        'd_ff': 256,          # FFN隐藏维度 (2 * d_model, TimeMixer默认)
        'dropout': 0.1,
        'down_sampling_layers': 3,   # 下采样层数（创建4个尺度）
        'down_sampling_window': 2,   # 下采样窗口大小
    }

    modelName = f'geoEmbed_{dataParam["geoMethod"]}_TimeMixer_meanteacher_{dataParam["geoFeatures"]}Geo'
    model_path = os.path.join(output_dir, modelName)
    print("步骤3: 初始化学生模型和教师模型（TimeMixer）")

    # 初始化学生模型
    student_model = TimeMixerModel(modelParam).to(device)
    # 初始化教师模型（作为学生模型的副本）
    teacher_model = TimeMixerModel(modelParam).to(device)
    # 复制学生模型的初始参数到教师模型
    ...
    with open(f'./{model_path}_log', 'a') as f:
        print("="*60, file=f)
        print("TimeMixer + Mean Teacher 配置:", file=f)
        ...
        print("  Backbone: TimeMixer (ICLR 2024)", file=f)
        print(f"  TimeMixer参数: d_model={modelParam['HLD']}, "
              f"e_layers={modelParam['nGNN']}, d_ff={modelParam['d_ff']}, "
              f"dropout={modelParam['dropout']}", file=f)
        print(f"  多尺度: down_sampling_layers={modelParam['down_sampling_layers']}, "
              f"window={modelParam['down_sampling_window']}", file=f)
        seq_lengths = [modelParam['iDim']]
        for _ in range(modelParam['down_sampling_layers']):
            seq_lengths.append(seq_lengths[-1] // modelParam['down_sampling_window'])
        print(f"  特征尺度: {seq_lengths}", file=f)
    ...
    ##----------------------训练----------------------
    print("步骤4: 开始 TimeMixer + Mean Teacher 训练")
