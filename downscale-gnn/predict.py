...
trainLoader, validLoader, _, validSet = data.dataGen(dataParam,data_path,predMode=True)
nNodes = validSet[0].x.shape[0]
model = (network.GNN(modelPara)).to(device)
network.loadCheckPoint(modelName,model,-1,device,predMode=True,output_dir=output_dir)
...
savemat(f'{output_dir}/full_gnn_pred_sens_{expAlgo}.mat',savePrediction)
