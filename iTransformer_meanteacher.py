"""
iTransformer + Mean Teacher 主程序
使用 iTransformer 替换 GNN 作为 backbone，保持 Mean Teacher 半监督学习框架
"""
import os
import torch
import pickle
...
from backbones import iTransformerModel  # 使用 iTransformer 替换 GNN
...
def main():
    ...
    # 创建输出目录：方法名_时间_jobid
    method_name = 'iTransformer_meanteacher'
    ...
    modelParam = {
        'HLD': 128,           # d_model
        'nMLP': 2,            # 保留（iTransformer不使用，但保持兼容）
        'nGNN': 3,            # encoder层数 (e_layers)
        'iDim': metadata['iDim'],
        'oDim': metadata['oDim'],
        # iTransformer 特有参数
    ...
    modelName = f'geoEmbed_{dataParam["geoMethod"]}_iTransformer_meanteacher_{dataParam["geoFeatures"]}Geo'
    model_path = os.path.join(output_dir, modelName)
    print("步骤3: 初始化学生模型和教师模型（iTransformer）")

    # 初始化学生模型
    student_model = iTransformerModel(modelParam).to(device)
    # 初始化教师模型（作为学生模型的副本）
    teacher_model = iTransformerModel(modelParam).to(device)
    # 复制学生模型的初始参数到教师模型
    ...
    wandb.init(
        ...
        config={
            **dataParam,
            **modelParam,
            'method': 'iTransformer + MeanTeacher',
    ...
    with open(f'./{model_path}_log', 'a') as f:
        print("="*60, file=f)
        print("iTransformer + Mean Teacher 配置:", file=f)
        ...
        print("  Backbone: iTransformer (ICLR 2024)", file=f)
        print(f"  iTransformer参数: d_model={modelParam['HLD']}, n_heads={modelParam['nhead']}, "
              f"e_layers={modelParam['nGNN']}, d_ff={modelParam['d_ff']}, dropout={modelParam['dropout']}", file=f)
    ...
    ##----------------------训练----------------------
    print("步骤4: 开始 iTransformer + Mean Teacher 训练")
    ...
    for epoch in range(EPOCH, nEpoch):
        ...
        # Mean Teacher训练
        # 注意：有标签数据=68站点，无标签数据=200站点，iTransformer需要知道两者的区别
